...
public:
  ...
  bool hit(const ray &r, interval ray_dist, hit_record &rec) const override {
    ...
    auto area_double = cross(e1, e2).length();

    // Skip outside triangle
    auto c = cross(e1, p - p0);
    if (dot(normal_vector, c) < 0)
      return false;
    rec.u = c.length() / area_double;

    c = cross(p2 - p1, p - p1);
    if (dot(normal_vector, c) < 0)
      return false;
    rec.v = c.length() / area_double;
    ...
    rec.t = t;
    rec.p = p;
    rec.set_face_normal(r, normal_vector);
    rec.mat = mat;

    return true;
  }

  vec3 get_normal() const { return normal_vector; }
